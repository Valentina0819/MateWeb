...
const Formulario = () => {
  ...
  // VALIDACIONES (Mantenemos tu logica pero optimizada)
  const validarNombre = (val) => (!val || val.trim().length < 2 ? 'Mi­nimo 2 caracteres.' : '')
  const validarApellido = (val) => (!val || val.trim().length < 2 ? 'Mi­nimo 2 caracteres.' : '')
  const validarEmail = (val) => (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val) ? 'Email invalido.' : '')
  const validarContraseña = (val) => {
    if (!val || val.length < 6) return 'Minimo 6 caracteres.'
    if (!/(?=.*[A-Z])(?=.*\d)/.test(val)) return 'Usa Mayascula y numero.'
    return ''
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMensaje({ text: 'Error de conexion.', color: 'danger' })
    }
    setEnviando(false)
  }
    ...
    <>
      <style>
        {`
          ...
          .sym-lg { font-size: 8rem; opacity: 0.25; filter: blur(1px); } 
