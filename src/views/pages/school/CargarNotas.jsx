...
const CargarNotasDocente = () => {
  ...
  useEffect(() => {
    const fetchMaterias = async () => {
      ...
      const res = await fetch("http://localhost:4000/docente/materias-estudiantes", {
  ...
  const fetchHistorial = async (id_materia_inscrita) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`http://localhost:4000/evaluaciones/${id_materia_inscrita}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const data = await res.json();
    ...
    (materiaSeleccionada.estudiantes || []).forEach(est => {
      ...
      [1, 2, 3].forEach(lapso => {
        ...
        if (datos.nota !== undefined && datos.nota !== "" && corteHabilitado(lapso - 1)) {
          promises.push(
            fetch("http://localhost:4000/docente/guardar-nota", {
              method: "POST",
                          ...
                          <React.Fragment key={lapso}>
                            ...
                            <CTableDataCell>
                              <CFormInput
                                ...
                                onChange={e => handleNotaChange(id, lapso, "descripcion", e.target.value)}
                                disabled={
                                  !corteHabilitado(lapso - 1) ||
                                  (!estaEditando)
                                }
