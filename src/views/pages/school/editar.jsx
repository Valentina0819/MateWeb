...
const AsignacionesDocenteAdmin = () => {
  ...
  const fetchAsignaciones = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:4000/asignaciones-docente`);
  ...
  const fetchCatalogos = async () => {
    try {
      const [docRes, matRes, secRes, anioRes] = await Promise.all([
        ...
        fetch("http://localhost:4000/materias-anio"),
        fetch("http://localhost:4000/secciones"),
        fetch("http://localhost:4000/aniosescolares")
      ]);
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`http://localhost:4000/asignaciones-docente/${asignacionDelete.id_asignacion}`, {
        method: "DELETE"
      });
      const data = await res.json();
  ...
  const handleEdit = async () => {
    try {
      const res = await fetch(`http://localhost:4000/asignaciones-docente/${asignacionEdit.id_asignacion}`, {
        method: "PUT",
